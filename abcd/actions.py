...
class Extract(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if tracker.get_slot('refreshment') in ['without refreshments','without refreshment']:
            ...
            refReq = False
        if tracker.get_slot('vc') == 'with video conference':
            #SlotSet("videoConferenceReq", True)
            vcReq = True
        if tracker.get_slot('vc') == 'without video conference':
            vcReq = False
...
class BookRoomForm(FormAction):
    ...
    def slot_mappings(self):
        ...
        return {
                "roomType": [self.from_entity(entity="roomType" , intent="inform_room_booking"),self.from_text()],
                ...
                "videoConferenceReq": [self.from_intent(intent='affirm',value=True),
                            self.from_intent(intent='deny',value = False),self.from_text()],
                "refreshmentReq": [self.from_intent(intent='affirm',value=True),
                            self.from_intent(intent='deny',value = False),self.from_text()],
                "additionalInfoReq": [self.from_intent(intent='deny', value = False),
                            self.from_intent(intent='affirm',value=True),self.from_text()],
                "additionalInfo": [self.from_intent(intent='deny', value = True),self.from_intent(intent='affirm', value = True),self.from_text()],
                "projectCode": [self.from_text(),self.from_entity(entity = "projectCode")]
                }
    ...
    @staticmethod
    def validate_additionalInfo(value: Text,
                                 dispatcher: CollectingDispatcher,
                                 tracker: Tracker,
                                 domain: Dict[Text, Any]) -> Any:
        if value in [True,False]:
            dispatcher.utter_template('utter_wrong_additionalInfo', tracker)
